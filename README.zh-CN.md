...
面向 Well Insight 的 Vue 3 组件库：带主题的表单、浮层、数据展示与反馈等基础控件。
...
## 应用级默认配置（`createWellInsight`）

可选的 Vue 插件，用于设置全局默认值（浮层挂载、尺寸、密度、语言、z-index）：
...
| 导入 | 用途 |
| --- | --- |
| `@well-insight/ui` | 组件（`WdButton`、`WdTable` 等）、`createWellInsight`、`WdConfigProvider`、主题与语言、`message` / `toast` |
...
pnpm build        # 组件库构建 → dist/
pnpm test
pnpm typecheck
```

维护者说明（构建、发版、提交规范）：[docs/DEVELOPMENT.zh-CN.md](./docs/DEVELOPMENT.zh-CN.md) · [UI 开发](./docs/ui-development.zh-CN.md) · [发版脚本](./scripts/README.md)。
