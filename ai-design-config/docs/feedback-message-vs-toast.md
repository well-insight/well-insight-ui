...
| **WdMessage 组件** | 页面内嵌条，不自动消失 | `<WdMessage severity="error">…</WdMessage>` | 表单/认证区**常驻**错误、警告 |

## 决策树（按顺序判断）

```
用户操作完成，需要即时反馈？
├─ 否 → 不需要 Message/Toast（可能用 WdConfirmDialog / 字段 errorMessage）
└─ 是 → 错误需留在表单区域直到用户修正？
    ├─ 是 → <WdMessage> 或字段级 invalid / errorMessage
...
```ts
import { message } from '@wex-design/ui'
...
```vue
<WdMessage v-if="error" severity="error" :closable="false">
  {{ error }}
</WdMessage>
```
...
| 表单校验失败只弹 Toast | 字段 `errorMessage` 或 `<WdMessage>` |
| 删除确认用 Message/Toast | `WdConfirmDialog` |
...
| `<WdMessage>` | 页面内 `el-alert` / 表单顶部错误条 |
