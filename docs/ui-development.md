...
Maintainer notes for `@wex-design/ui` (build, docs playground, publish). External consumers should use the [root README](../README.md). Contributor setup: [DEVELOPMENT.md](./DEVELOPMENT.md).
...
Output is under `dist/` (`index.js`, `index.d.ts`, `styles.css`, `resolver.js`, plus per-component kebab-case subpaths such as `button/index.js`). Subpath entries such as `@wex-design/ui/button` include JS, dependencies, and styles.
...
- **Full entry** (`src/index.ts` → `dist/index.js`): re-exports `.vue` files directly, not component `index.ts`, so on-demand style side-effects stay out of the main bundle; pair with `@wex-design/ui/styles.css`.
...
Run step-by-step or use the orchestrator. See [scripts/README.md](../scripts/README.md). By default this publishes both `@wex-design/ui` and `@wex-design/ui-mcp`:
...
`@wex-design/ui-mcp` is an optional stdio server for clients that support [MCP](https://modelcontextprotocol.io/) (it does not replace installing `@wex-design/ui`). Public docs: docs site [MCP](/docs/mcp). Implementation: [packages/ui-mcp/README.md](../packages/ui-mcp/README.md).
