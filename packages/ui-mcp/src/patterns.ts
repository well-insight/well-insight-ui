...
export const pagePatterns: PagePattern[] = [
  {
    ...
    structure: [
      'WdConfigProvider',
      '└── WdLayout (has-sider)',
      '    ├── WdLayoutSider → WdMenu (optional)',
      '    └── WdLayout',
      '        ├── WdLayoutHeader → WdBreadcrumb',
      '        └── WdLayoutContent',
      '            ├── Filter section (WdSpace + WdInput + WdSelect + WdButton)',
      '            ├── PageHeader (title + primary action)',
      '            └── WdTable (paginator, #cell-* slots, #empty)',
    ],
    layout: {
      page: 'WdLayout has-sider 作为后台骨架；内容区 padding 使用 --wd-space-6',
      header: 'WdLayoutHeader 放面包屑；标题与主操作在内容区 toolbar 两端对齐',
      filters: '筛选区使用浅色背景 + border-radius；桌面端 WdSpace wrap，窄屏堆叠',
      data: 'WdTable 启用 paginator；行操作用 WdButton size="small"；空态用 #empty 插槽',
    },
    styleRules: [
      '新增、保存等主操作使用 primary WdButton',
      '查询等次要操作使用 secondary 或 outlined WdButton',
      '删除使用 danger WdButton，并配合 WdDialog 或 WdConfirmDialog',
      '业务状态使用 WdTag，不使用 Button severity 表达状态',
      '间距、颜色、圆角优先使用 --wd-* 设计令牌，不直接硬编码色板',
    ],
    interactionRules: [
      '筛选提交后重置分页到第一页',
      '加载中禁用重复提交并显示 loading 状态',
      '空数据使用 WdTable #empty 插槽或 DataView 空态，不渲染空白表格',
      '行级危险操作必须有确认反馈',
      'CRUD 成功/删除等单行回执使用 message API，不要默认 Toast',
    ],
    avoid: [
      '不要用 WdDialog 承载完整列表页',
      '不要用 WdButton 代替业务状态标签',
  ...
  },
  {
    ...
    components: [
      ...
      { component: 'FormItem', role: '字段标签、帮助文本和校验状态', required: false, reason: '由 WdForm 同目录导出，作为表单子结构使用' },
    ...
    layout: {
      shell: 'WdLayout + WdLayoutHeader + WdLayoutContent；表单 max-width 约 42rem',
    ...
    styleRules: [
      '统一使用 WdForm + WdFormItem 管理校验，不在页面中散落错误文案',
      '字段控件通常使用 fluid，宽度由表单布局控制',
      '提交使用 primary WdButton，保存中传入 loading',
      '取消使用 secondary text WdButton，避免与提交按钮争夺视觉层级',
      '使用 WdConfigProvider 或 --wd-* Token 保持尺寸和间距一致',
    ],
    interactionRules: [
  ...
  },
  {
    ...
    layout: {
      shell: '使用 WdLayout + WdLayoutHeader + WdLayoutContent',
      page: '以 WdGrid 组织指标卡和内容卡，避免用绝对定位拼大屏',
      kpi: '指标卡保持等宽，主指标、变化量和时间范围层级清晰',
      content: '图表和明细按业务优先级排列，重要告警靠前',
      responsive: '多列网格在窄屏降为单列或两列',
    },
    styleRules: ['指标使用 text hierarchy，不用大面积高饱和背景', '状态和告警使用 WdTag 等语义组件', '图表容器使用 Card 和 --wd-* Token 保持表面一致', '实时刷新操作使用 secondary 或 text WdButton'],
  ...
  },
  {
    ...
    styleRules: ['开关用于二元启用状态，不用 Select 模拟开关', '保存使用 primary，恢复默认值使用 secondary 或 text', '危险配置需要明确说明影响范围并使用 danger 语义', '使用 WdConfigProvider 统一 size、density 和 locale'],
  ...
  },
  {
    ...
    styleRules: ['空态文案使用中性、可行动的语气', '主恢复动作使用 primary WdButton，次动作使用 text', '不要用错误色表达正常的无数据状态'],
  ...
  },
  {
    ...
    components: [
      ...
      { component: 'FormItem', role: '字段标签和错误信息', required: false, reason: '由 WdForm 同目录导出，作为表单子结构使用' },
    ...
    styleRules: ['密码字段使用 InputPassword 或 documented password API', '提交按钮使用 primary + fluid', '不要用高对比装饰削弱错误信息和焦点状态', '品牌视觉可定制，但控件状态仍使用 --wd-* Token'],
  ...
  },
  {
    ...
    styleRules: [
      '状态使用 WdTag 的语义色',
      '内容分组使用 WdCard，必要时用 WdDivider 分隔',
      '编辑使用 primary 或 outlined WdButton，删除使用 danger',
...
export const designRules = {
  ...
  actions: {
    primary: { component: 'WdButton', props: ['severity omitted or primary'] },
  ...
  global: [
    '优先使用组件库组件和 --wd-* Token，不重复维护第二套色板',
