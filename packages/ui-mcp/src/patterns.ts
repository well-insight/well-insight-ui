...
export const pagePatterns: PagePattern[] = [
  {
    ...
    structure: [
      'WiConfigProvider',
      '└── WiLayout (has-sider)',
      '    ├── WiLayoutSider → WiMenu (optional)',
      '    └── WiLayout',
      '        ├── WiLayoutHeader → WiBreadcrumb',
      '        └── WiLayoutContent',
      '            ├── Filter section (WiSpace + WiInput + WiSelect + WiButton)',
      '            ├── PageHeader (title + primary action)',
      '            └── WiTable (paginator, #cell-* slots, #empty)',
    ],
    layout: {
      page: 'WiLayout has-sider 作为后台骨架；内容区 padding 使用 --wi-space-6',
      header: 'WiLayoutHeader 放面包屑；标题与主操作在内容区 toolbar 两端对齐',
      filters: '筛选区使用浅色背景 + border-radius；桌面端 WiSpace wrap，窄屏堆叠',
      data: 'WiTable 启用 paginator；行操作用 WiButton size="small"；空态用 #empty 插槽',
    },
    styleRules: [
  ...
  },
  {
    ...
    layout: {
      shell: 'WiLayout + WiLayoutHeader + WiLayoutContent；表单 max-width 约 42rem',
  ...
  },
  {
    ...
    layout: {
      shell: '使用 WiLayout + WiLayoutHeader + WiLayoutContent',
      page: '以 WiGrid 组织指标卡和内容卡，避免用绝对定位拼大屏',
