...
主题能力内置于 `@well-insight/ui`。组件只消费语义化 CSS 变量（`--wd-*`），不自行维护第二套色板。
...
应用级可用 `createWellInsight({ density: 'compact' })` 或 `<WdConfigProvider density="compact">`。
...
主题切换是「视觉层」；`WdConfigProvider` / `createWellInsight` 负责尺寸、文案、浮层挂载等「行为默认值」。二者可同时使用，详见 [全局配置](/docs/config)。
