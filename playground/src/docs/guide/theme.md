...
主题能力内置于 `@wex-design/ui`。组件只消费语义化 CSS 变量（`--wd-*`），不自行维护第二套色板。

引入 `@wex-design/ui/styles.css` 时已包含这些变量；主题 JS API（`useTheme` 等）从同一包导入。
...
应用级可用 `createWexDesign({ density: 'compact' })` 或 `<WdConfigProvider density="compact">`。
...
主题切换是「视觉层」；`WdConfigProvider` / `createWexDesign` 负责尺寸、文案、浮层挂载等「行为默认值」。二者可同时使用，详见 [全局配置](/docs/config)。
