...
describe('wdDatePicker', () => {
  ...
  it('picks a date range across two clicks', async () => {
    const wrapper = mount(WdDatePicker, {
  ...
  it('applies a shortcut', async () => {
    ...
    await wrapper.find('.wd-datepicker__input').trigger('click')
    await nextTick()
    ;(document.body.querySelector('.wd-datepicker__shortcut') as HTMLButtonElement).click()
    await nextTick()
  ...
  it('associates label with input via for/id', () => {
    const wrapper = mount(WdDatePicker, { props: { label: '开始日期', modelValue: null } })
    const label = wrapper.get('.wd-datepicker__label')
    const input = wrapper.get('.wd-datepicker__input')
  ...
  it('renders error/help feedback and wires aria-describedby', () => {
    ...
    const help = wrapper.get('.wd-datepicker__help')
    expect(help.text()).toBe('日期无效')
    expect(help.classes()).toContain('wd-datepicker__help--invalid')
    expect(wrapper.get('.wd-datepicker__input').attributes('aria-describedby')).toBe(help.attributes('id'))
  })
  ...
  it('supports full keyboard date selection in the grid', async () => {
    ...
    const grid = document.body.querySelector('.wd-datepicker__grid') as HTMLElement
    expect(grid.getAttribute('role')).toBe('grid')
    const day15 = document.body.querySelector('[data-wd-date="2024-01-15"]') as HTMLButtonElement
    ...
    await nextTick()
    const day16 = document.body.querySelector('[data-wd-date="2024-01-16"]') as HTMLButtonElement
    ...
    expect(document.body.querySelector('.wd-datepicker__panel')).toBeNull()
  ...
  it('exposes combobox semantics aligned with Select when open', async () => {
    ...
    expect(wrapper.get('.wd-datepicker__help').text()).toBe('Required')
    await input.trigger('click')
    await nextTick()
    const panel = document.body.querySelector('.wd-datepicker__panel') as HTMLElement
