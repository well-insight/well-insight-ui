...
describe('wdDrawer', () => {
  ...
  it('applies width for side drawers', async () => {
    ...
    await nextTick()
    const pane = document.body.querySelector('.wd-drawer') as HTMLElement
  ...
  it('respects closeOnEsc=false and emits close', async () => {
    ...
    document.body.querySelector('.wd-drawer__close')!.dispatchEvent(new MouseEvent('click', { bubbles: true }))
    await nextTick()
