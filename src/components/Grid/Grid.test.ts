...
describe('wdGrid', () => {
  ...
  it('hides overflowing items when collapsed', async () => {
    const wrapper = mount(WdGrid, {
      ...
      slots: {
        default: () => [
          h(WdGridItem, { span: 2 }, () => 'A'),
          h(WdGridItem, { span: 2 }, () => 'B'),
          h(WdGridItem, { span: 2 }, () => 'C'),
        ],
      },
    })
    await wrapper.vm.$nextTick()
    const items = wrapper.findAll('.wd-grid-item')
  ...
  it('applies offset into grid-column span like naive-ui', async () => {
    const wrapper = mount(WdGrid, {
      ...
      slots: {
        default: () => [
          h(WdGridItem, { span: 1, offset: 2 }, () => 'A'),
          h(WdGridItem, { span: 1, offset: 1 }, () => 'B'),
        ],
      },
    })
    await wrapper.vm.$nextTick()
  ...
  it('resolves item span responsive strings when itemResponsive is on', async () => {
    const wrapper = mount(WdGrid, {
      ...
      slots: {
        default: () => [h(WdGridItem, { span: '1 m:4' }, () => 'A')],
      },
      attachTo: document.body,
    })
    Object.defineProperty(wrapper.get('.wd-grid').element, 'clientWidth', {
      configurable: true,
      get: () => 1200,
    })
    await wrapper.setProps({ xGap: 8 })
    await wrapper.vm.$nextTick()
    expect(wrapper.find('.wd-grid-item').attributes('style')).toContain('grid-column: span 4 / span 4')
    wrapper.unmount()
  })
