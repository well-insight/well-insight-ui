...
describe('wdInputOtp', () => {
  ...
  it('masks digits', () => {
    const wrapper = mount(WdInputOtp, { props: { modelValue: '12', mask: true } })
    expect(wrapper.findAll('.wd-inputotp__input')[0]!.attributes('type')).toBe('password')
    expect(wrapper.classes()).toContain('wd-inputotp--mask')
  })
})
