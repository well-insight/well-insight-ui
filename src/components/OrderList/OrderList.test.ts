...
describe('wdOrderList', () => {
  ...
  it('moves selection with arrow keys and reorders with Ctrl+Arrow', async () => {
    const wrapper = mount(WdOrderList, {
    ...
    const list = wrapper.get('.wd-orderlist__list')
  ...
  it('shows empty message when list is empty', () => {
    const wrapper = mount(WdOrderList, { props: { modelValue: [] } })
    expect(wrapper.find('.wd-orderlist__message').exists()).toBe(true)
    expect(wrapper.find('.wd-orderlist__empty-text').text()).toBe('暂无数据')
  })
})
