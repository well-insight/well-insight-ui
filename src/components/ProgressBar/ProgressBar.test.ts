...
describe('wdProgressBar', () => {
  ...
  it('renders circle type and status tone', () => {
    const wrapper = mount(WdProgressBar, { props: { value: 40, type: 'circle', status: 'success' } })
    expect(wrapper.classes()).toEqual(expect.arrayContaining(['wd-progressbar--circle', 'wd-progressbar--success']))
    expect(wrapper.find('.wd-progressbar__circle').exists()).toBe(true)
    expect(wrapper.get('.wd-progressbar__circle-label').text()).toBe('40%')
  })
})
