...
describe('wdRating', () => {
  ...
  it('supports half stars and allowClear alias', async () => {
    const wrapper = mount(WdRating, { props: { modelValue: 1, allowHalf: true, allowClear: false } })
    expect(wrapper.find('.wd-rating__cancel').exists()).toBe(false)
    const star = wrapper.findAll('.wd-rating__star')[1]!
    await star.trigger('click', { clientX: 0 })
    expect(wrapper.emitted('update:modelValue')?.at(-1)?.[0]).toBeTypeOf('number')
    expect(wrapper.get('.wd-rating').classes()).toContain('wd-rating--half')
  })

  it('exposes slider semantics with label and value text', () => {
    const wrapper = mount(WdRating, { props: { modelValue: 3, stars: 5 } })
  ...
  it('does not respond to keys when readonly', async () => {
    const wrapper = mount(WdRating, { props: { modelValue: 2, readonly: true } })
