...
```ts
import { WdSelect } from '@wex-design/ui'
```
...
<template>
  <div style="display:grid;gap:1rem;width:min(24rem,100%)">
    <WdSelect v-model="value" :options="options" multiple show-clear placeholder="Teams" />
    <WdSelect v-model="value" :options="options" multiple :max-tag-count="1" placeholder="Collapsed tags" />
  </div>
</template>
```
...
<template>
  <WdSelect v-model="value" :options="options" multiple filter tag placeholder="Add a stack" />
</template>
```
...
<template>
  <div style="display:grid;gap:1rem;width:min(24rem,100%)">
    <WdSelect v-model="value" :options="[]" empty-message="暂无选项" placeholder="Empty list" />
