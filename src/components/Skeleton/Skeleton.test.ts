...
describe('wdSkeleton', () => {
  ...
  it('repeats text placeholders', () => {
    const wrapper = mount(WdSkeleton, { props: { text: true, repeat: 3 } })
    expect(wrapper.classes()).toContain('wd-skeleton-repeat')
    expect(wrapper.findAll('.wd-skeleton--text')).toHaveLength(3)
  })
})
